// ignore_for_file: avoid_print, unused_local_variable, unused_element, prefer_interpolation_to_compose_strings, sized_box_for_whitespace
...
class UserViewNotices extends StatefulWidget {
  final String studentEmail;
  final String studentId; //  Add these as final variables
  final String studentClass;

  const UserViewNotices({
    super.key,
  ...
  @override
  _UserViewNoticesState createState() => _UserViewNoticesState();
}

class _UserViewNoticesState extends State<UserViewNotices> {
  ...
  Future<void> _fetchStudentClassAndNotices() async {
    try {
      ...
      if (studentQuery.docs.isNotEmpty) {
        ...
        // Fetch notices
        QuerySnapshot noticesQuery = await FirebaseFirestore.instance
            .collection('notices')
            .where('class', isEqualTo: studentClass)
            .get();

        List<Map<String, dynamic>> fetchedNotices =
            noticesQuery.docs.map((doc) {
        ...
        fetchedNotices.sort((a, b) => b['date'].compareTo(a['date']));

        setState(() {
          notices = fetchedNotices;
          filteredNotices = fetchedNotices;
          isLoading = false;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigation(
        ...
        selectedIndex: 4, // Change based on the current page
      ),
      body: isLoading
                  ...
                  child: filteredNotices.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            var note = filteredNotices[index];
